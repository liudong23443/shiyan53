                    ...
                    with st.spinner("正在生成SHAP解释图..."):
                        ...
                        for i, row in importance_df.iterrows():
                            ...
                            if feature in feature_values:
                                ...
                                feature_labels.append(f"{value_display} = {feature}")
                            else:
                                feature_labels.append(feature)
                        
                        # 创建颜色列表 - 红色为正(增加风险)，蓝色为负(降低风险)
                        colors = ['#ff4d4d' if x > 0 else '#2196F3' for x in importance_df['SHAP值']]
                        
                        # 绘制水平条形图
                        bars = plt.barh(importance_df.index, importance_df['SHAP值'], color=colors, alpha=0.8)
                        ...
                        # 设置Y轴标签为自定义特征标签
                        plt.yticks(importance_df.index, feature_labels)
                        
                        # 设置X轴范围
                        plt.xlim(-max(abs(importance_df['SHAP值'])) * 1.2, max(abs(importance_df['SHAP值'])) * 1.2)
                        ...
                        plt.xlabel("SHAP值 (负值=降低风险, 正值=增加风险)", fontsize=10)
                        
                        # 添加基准值说明
                        plt.figtext(0.1, 0.02, f"基准值 f(x) = {base_value_formatted}", fontsize=9)
                        ...
                        # 紧凑布局
                        plt.tight_layout(rect=[0, 0.05, 1, 0.95])  # 预留位置给基准值说明
                        
                        # 保存并显示图
                        plt.savefig("shap_effect_plot.png", dpi=200, bbox_inches='tight')
                        plt.close()
                        st.image("shap_effect_plot.png")
